...
SELECT id, workspace_id, issue_id, comment_id, uploader_type, uploader_id, filename, url, content_type, size_bytes, created_at, chat_session_id, chat_message_id, updated_at FROM attachment
...
const updateAttachmentContentIfMatch = `-- name: UpdateAttachmentContentIfMatch :one
UPDATE attachment
SET size_bytes = $3, content_type = $4, updated_at = now()
...
type UpdateAttachmentContentIfMatchParams struct {
	ID          pgtype.UUID        `json:"id"`
	WorkspaceID pgtype.UUID        `json:"workspace_id"`
	SizeBytes   int64              `json:"size_bytes"`
	ContentType string             `json:"content_type"`
...
func (q *Queries) UpdateAttachmentContentIfMatch(ctx context.Context, arg UpdateAttachmentContentIfMatchParams) (Attachment, error) {
	row := q.db.QueryRow(ctx, updateAttachmentContentIfMatch,
		arg.ID,
		arg.WorkspaceID,
		arg.SizeBytes,
		arg.ContentType,
